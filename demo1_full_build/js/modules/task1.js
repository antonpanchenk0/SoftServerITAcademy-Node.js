...
export function createCheeseBoard(height, width, char) {
    ...
    if(typeof height != "number" || typeof width != "number" || typeof char != "string" || isNaN(height)) return {status: 'failed', reason: "Не верные параметры функции. createCheeseBoard(height(type:Number), width(type:Number), char(type:String))."};
